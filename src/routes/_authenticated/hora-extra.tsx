...
function OvertimePage() {
  ...
  const [tab, setTab] = useState<"list" | "queue" | "new">(
    canRequest ? "list" : "queue",
  );
  ...
  return (
    <main className="mx-auto max-w-[1400px] px-4 py-6 sm:px-6">
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
        ...
        <KpiCard label="Aprovadas" value={kpis.approved} tone="success" icon={<CheckCircle2 className="h-3.5 w-3.5" />} />
        <KpiCard label="Reprovadas" value={kpis.rejected} tone="destructive" icon={<XCircle className="h-3.5 w-3.5" />} />
        <KpiCard label="Lanches necessários" value={kpis.snacks} tone="primary" icon={<Utensils className="h-3.5 w-3.5" />} />
      </div>

      <div className="mb-3 inline-flex rounded-md border border-border bg-card p-1 text-[12px]">
        {canRequest && (
        ...
        {isManager && (
          <TabBtn active={tab === "queue"} onClick={() => setTab("queue")}>
            Aprovações {kpis.pending > 0 && <span className="ml-1 rounded bg-warning/20 px-1 text-[10px] text-warning-foreground">{kpis.pending}</span>}
          </TabBtn>
        )}
      </div>

      {tab === "list" && canRequest && (
        <MyRequests rows={rows.filter((r) => r.requester_user_id === s.userId)} onCancel={async (row) => {
...
function MyRequests({ rows, onCancel }: { rows: OvertimeRow[]; onCancel: (r: OvertimeRow) => void }) {
  if (rows.length === 0) {
    return (
      <Panel padded={false}>
        <div className="p-6"><EmptyState icon={<Timer className="h-4 w-4" />} title="Nenhuma solicitação" description="Clique em Nova solicitação para começar." /></div>
      </Panel>
    );
  }
  return (
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (q) {
        ...
        const hay = `${r.employee_name} ${r.employee_registration} ${r.requester_name} ${r.order_number ?? ""} ${r.service_description}`.toLowerCase();
    ...
    <>
        ...
        <div className="flex flex-wrap items-end gap-2 border-b border-border p-3">
          <Field label="Buscar">
            <div className="relative">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Colaborador, matrícula, ordem…" className="input-base pl-7 text-[12px]" />
            </div>
          </Field>
          <Field label="Status">
            <select value={status} onChange={(e) => setStatus(e.target.value as any)} className="input-base text-[12px]">
          ...
          <Field label="De"><input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="input-base text-[12px]" /></Field>
          <Field label="Até"><input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="input-base text-[12px]" /></Field>
        </div>
        {filtered.length === 0 ? (
          <div className="p-6"><EmptyState icon={<ListChecks className="h-4 w-4" />} title="Nenhuma solicitação" /></div>
        ) : (
          <RequestsTable
            rows={filtered}
            showRequester
      ...
      {decideRow && (
        <DecideModal
          ...
          onDone={() => { setDecideRow(null); onDecided(); }}
        />
      )}
    </>
  );
...
/* ---------- Table ---------- */
function RequestsTable({ rows, showRequester, onApprove, onReject, onCancel }: {
  rows: OvertimeRow[];
        ...
        <tbody className="divide-y divide-border/60">
          {rows.map((r) => (
            <tr key={r.id} className="row-zebra align-top">
              ...
              <td className="px-3 py-2 tabular">{r.order_number || <span className="text-muted-foreground">—</span>}</td>
              <td className="px-3 py-2 max-w-[240px]"><div className="line-clamp-2">{r.service_description}</div></td>
              ...
              <td className="px-3 py-2"><OvertimeStatus status={r.status} /></td>
              <td className="px-3 py-2 text-[11px]">
                {r.decided_at ? (
                  <>
                    <div className="font-medium">{r.decided_by_name}</div>
                    <div className="text-muted-foreground">{formatDateTime(r.decided_at)}</div>
                    {r.manager_comment && <div className="mt-1 max-w-[240px] text-muted-foreground">"{r.manager_comment}"</div>}
                  </>
              ...
              <td className="px-3 py-2 text-right">
                <div className="inline-flex flex-wrap justify-end gap-1">
                  {onApprove && r.status === "pending" && (
                    <button onClick={() => onApprove(r)} className="btn-success py-1 text-[11px]">Aprovar</button>
                  )}
                  {onReject && r.status === "pending" && (
                    <button onClick={() => onReject(r)} className="inline-flex items-center gap-1 rounded-md border border-destructive/40 bg-destructive/10 px-2 py-1 text-[11px] font-medium text-destructive hover:bg-destructive/15">Reprovar</button>
                  )}
                  {onCancel && r.status === "pending" && (
                    <button onClick={() => onCancel(r)} className="inline-flex items-center gap-1 rounded-md border border-border bg-card px-2 py-1 text-[11px] font-medium text-muted-foreground hover:bg-muted">Cancelar</button>
                  )}
                </div>
              </td>
            </tr>
...
function NewRequestModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const activeWeek = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("weeks").select("id").eq("is_active", true).maybeSingle();
      return data;
    },
  });
    ...
    <Modal onClose={onClose} title="Nova solicitação de hora extra" size="lg">
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Nome do colaborador" required>
          <input className="input-base" value={form.employee_name} onChange={(e) => setForm({ ...form, employee_name: e.target.value })} />
        </Field>
        <Field label="Matrícula" required>
          <input className="input-base" value={form.employee_registration} onChange={(e) => setForm({ ...form, employee_registration: e.target.value })} />
        </Field>
        <Field label="Função" required>
          <input className="input-base" value={form.employee_role} onChange={(e) => setForm({ ...form, employee_role: e.target.value })} />
        </Field>
        <Field label="Data da hora extra" required>
          <input type="date" className="input-base" value={form.overtime_date} onChange={(e) => setForm({ ...form, overtime_date: e.target.value })} />
        </Field>
        <Field label="Horário de saída" required>
          <input type="time" className="input-base" value={form.departure_time} onChange={(e) => setForm({ ...form, departure_time: e.target.value })} />
        </Field>
        <Field label="Precisa de lanche?">
          <select className="input-base" value={form.needs_snack ? "1" : "0"} onChange={(e) => setForm({ ...form, needs_snack: e.target.value === "1" })}>
      ...
      <div className="mt-4 rounded-md border border-border bg-muted/30 p-3">
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Vincular atividade da semana ativa (opcional)</div>
        <div className="relative">
          <Search className="pointer-events-none absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
          <input
        ...
        {form.activity_id && (
          <div className="mt-2 flex items-center justify-between rounded border border-primary/30 bg-primary/5 p-2 text-[12px]">
            <span>Vinculada: <b className="tabular">{form.order_number}</b></span>
            <button type="button" onClick={() => setForm({ ...form, activity_id: null, week_id: null })} className="text-[11px] text-muted-foreground hover:text-foreground">Remover vínculo</button>
          </div>
        )}
      </div>
      ...
      <div className="mt-3 grid gap-3">
        <Field label="Número da ordem">
          <input className="input-base" value={form.order_number} onChange={(e) => setForm({ ...form, order_number: e.target.value })} placeholder="Preenchido ao vincular, ou digite manualmente" />
        </Field>
        <Field label="Serviço / atividade" required>
          <textarea rows={2} className="input-base" value={form.service_description} onChange={(e) => setForm({ ...form, service_description: e.target.value })} />
        </Field>
        <Field label="Justificativa" required>
          <textarea rows={3} className="input-base" value={form.justification} onChange={(e) => setForm({ ...form, justification: e.target.value })} />
        </Field>
      </div>

      <div className="mt-4 flex justify-end gap-2">
        <button onClick={onClose} className="rounded-md border border-border bg-card px-3 py-1.5 text-[12px] hover:bg-muted">Cancelar</button>
        <button onClick={submit} disabled={saving} className="btn-primary text-[12px] disabled:opacity-60">
...
/* ---------- Decide Modal ---------- */
function DecideModal({ row, decision, onClose, onDone }: {
  row: OvertimeRow;
  ...
  async function submit() {
    ...
    if (!confirm(isReject ? `Reprovar solicitação #${row.request_number}?` : `Aprovar solicitação #${row.request_number}?`)) return;
    setSaving(true);
    const res = await call({ data: { id: row.id, expectedVersion: row.version, decision, comment: comment.trim() || null } });
    setSaving(false);
    if (!res.ok) {
  ...
  return (
    <Modal onClose={onClose} title={isReject ? "Reprovar solicitação" : "Aprovar solicitação"}>
      <div className="rounded-md border border-border bg-muted/30 p-3 text-[12px]">
        <div><b>#{row.request_number}</b> · {formatDate(row.overtime_date)}</div>
        <div className="mt-1"><b>Colaborador:</b> {row.employee_name} · {row.employee_registration} · {row.employee_role}</div>
        <div><b>Ordem:</b> {row.order_number || "—"} · <b>Saída:</b> {row.departure_time} · <b>Lanche:</b> {row.needs_snack ? "Sim" : "Não"}</div>
        <div className="mt-1"><b>Serviço:</b> {row.service_description}</div>
        <div className="mt-1"><b>Justificativa:</b> {row.justification}</div>
      ...
      <div className="mt-4 flex justify-end gap-2">
        <button onClick={onClose} className="rounded-md border border-border bg-card px-3 py-1.5 text-[12px] hover:bg-muted">Cancelar</button>
        <button onClick={submit} disabled={saving} className={cn(isReject ? "inline-flex items-center gap-1 rounded-md bg-destructive px-3 py-1.5 text-[12px] font-medium text-destructive-foreground hover:opacity-90 disabled:opacity-60" : "btn-success text-[12px] disabled:opacity-60")}>
