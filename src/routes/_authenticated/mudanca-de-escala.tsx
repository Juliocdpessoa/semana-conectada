...
export const Route = createFileRoute("/_authenticated/mudanca-de-escala")({
  beforeLoad: ({ context }) => {
    const s = (context as { session: SessionInfo }).session;
    if (!s.roles.some((role) => ALLOWED_ROLES.includes(role)))
...
function ScheduledTransportPage() {
  ...
  const availableDates = useMemo(
    () => (query.data?.options?.dates ?? []) as string[],
    [query.data?.options?.dates],
  );
  ...
  async function exportExcel() {
    try {
      ...
      const exportBatches = new Map(
        ((exportResult.batches ?? []) as ScheduledTransportBatch[]).map((batch) => [
          batch.id,
          batch,
        ]),
      );
      ...
      const headers = isLogistics
        ? LOGISTICS_SCHEDULED_TRANSPORT_EXPORT_HEADERS
        : SCHEDULED_TRANSPORT_EXPORT_HEADERS;
      const widths = isLogistics
        ? LOGISTICS_SCHEDULED_TRANSPORT_EXPORT_WIDTHS
        : SCHEDULED_TRANSPORT_EXPORT_WIDTHS;
  ...
  async function cancelSelected() {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Não foi possível cancelar as mudanças de escala.",
      );
    }
  }
  ...
  async function cancelOne(row: ScheduledTransportRow) {
    if (
      !confirm(
        `Cancelar a mudança de escala de ${row.employee_name} em ${formatDate(row.transport_date)}?`,
      )
    )
      return;
    try {
      ...
      if (!result.ok) return toast.error(result.error);
      if (result.count === 0)
        return toast.error("A mudança de escala já foi alterada ou cancelada.");
      toast.success("Mudança de escala cancelada.");
      queryClient.invalidateQueries({ queryKey: ["scheduled-transport"] });
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Não foi possível cancelar a mudança de escala.",
      );
    }
  }
          ...
          !showDaysOff ? (
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={exportExcel}
                className="btn-primary min-h-9 text-[12px]"
              >
      ...
      {!showDaysOff && (
        <>
          <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
            <KpiCard
              label="Colaboradores programados"
            ...
            <KpiCard
              ...
              value={kpis.noTransport}
              icon={<Users className="h-4 w-4" />}
            />
            <KpiCard
              label="Colaboradores cancelados"
            ...
            >
              <div className="grid min-w-0 grid-cols-1 gap-2 border-b border-border p-3 sm:grid-cols-2 lg:grid-cols-4 [&>*]:min-w-0">
                ...
                <Field label="Função">
                  <select
                    ...
                    onChange={(event) =>
                      setFilters((f) => ({ ...f, jobTitle: event.target.value }))
                    }
                    className="input-base block w-full min-w-0 max-w-full text-[16px] sm:text-[12px]"
                  >
                    <option value="">Todas</option>
                ...
                <div className="grid min-w-0 grid-cols-2 gap-2 [&>*]:min-w-0">
                  ...
                  <Field label="Saída">
                    <select
                      ...
                      onChange={(event) =>
                        setFilters((f) => ({ ...f, departureTime: event.target.value }))
                      }
                      className="input-base block w-full min-w-0 max-w-full text-[16px] sm:text-[12px]"
                    >
                      <option value="">Todos</option>
              ...
              {query.isLoading ? (
                ...
                <div className="p-6">
                  <EmptyState
                    icon={<Bus className="h-4 w-4" />}
                    title="Nenhuma mudança de escala encontrada"
                  />
                </div>
              ) : (
                <>
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            ...
                            <div className="text-[11px] text-muted-foreground">
                              {formatDate(row.transport_date)} · {row.entry_time}–
                              {row.departure_time}
                            </div>
                          </div>
                  ...
                  {totalRows > pageSize && (
                    <div className="flex flex-col gap-2 border-t border-border bg-muted/20 px-3 py-3 text-[12px] text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                      <span>
                        Página {currentPage} de {pageCount} · {totalRows} registro(s) · até{" "}
                        {pageSize} por página
                      </span>
...
function DaysOffControl({
  ...
  async function deleteRow(row: EmployeeDayOffRow) {
    if (!confirm(`Excluir a folga de ${row.employee_name} em ${formatDate(row.day_off_date)}?`))
      return;
      ...
      >
        ...
        {rowsQuery.isLoading ? (
          ...
          <div className="p-8 text-center text-sm text-destructive">
            Não foi possível carregar as folgas.
          </div>
        ) : filtered.length === 0 ? (
          <EmptyState
                ...
                <article key={row.id} className="rounded-lg border border-border bg-card p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {row.employee_registration || "Sem matrícula"} ·{" "}
                        {row.employee_role || "Sem função"}
                      </div>
                    </div>
                  ...
                  {row.observation && (
                    <p className="mt-2 text-[12px] text-muted-foreground">{row.observation}</p>
                  )}
...
function DayOffFormModal({
  ...
  const [employeeIds, setEmployeeIds] = useState<Set<string>>(
    () => new Set(row ? [row.employee_master_id] : []),
  );
  ...
  async function save() {
    ...
    try {
      ...
      if (errors.length) {
        toast.warning(
          "Alguns colaboradores já possuíam folga nessa data ou não puderam ser incluídos.",
        );
      }
      onSaved();
    } catch (error) {
        ...
        <div className="flex justify-end gap-2 border-t border-border pt-4">
          ...
          >
            {saving
              ...
              : row
                ? "Salvar alterações"
                : `Registrar ${employeeIds.size} folga(s)`}
          </button>
        </div>
      </div>
    </Modal>
...
function NewScheduleModal({
  ...
  const visible = useMemo(
    () => filterEmployees(employees, search).slice(0, 200),
    [employees, search],
  );
      ...
      {!preview ? (
        <div className="space-y-4">
          <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 [&>*]:min-w-0">
            ...
            <Field
              ...
              hint="Deixe em branco para programar somente a data inicial."
            >
              <input
                type="date"
                value={endDate}
        ...
        <div className="space-y-3 text-[13px]">
          <div className="rounded-md border border-border bg-muted/30 p-3">
            ...
            <div>
              <strong>Dias considerados:</strong>{" "}
    ...
    try {
      ...
      toast.success(
        result.count +
          ...
          (result.conflicts
            ? ` ${result.conflicts} conflito(s) foram preservados e não sobrescritos.`
            : ""),
      );
      onSaved();
    } finally {
    ...
    <Modal title={`Editar mudança de escala · ${row.employee_name}`} onClose={onClose}>
      <div className="space-y-3">
        <div className="text-[12px] text-muted-foreground">
          Data: {formatDate(row.transport_date)}
        </div>
        {row.batch_id && (
