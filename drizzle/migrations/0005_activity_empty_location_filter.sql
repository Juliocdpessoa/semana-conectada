...
WITH params AS (
  ...
  SELECT b.*,
    ...
    (jsonb_array_length(p.locations)=0 OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(p.locations) x WHERE (x='__EMPTY__' AND b.location_label IS NULL) OR upper(btrim(x))=upper(btrim(coalesce(b.location_label,''))))) pass_location,
...
SELECT jsonb_build_object(
  ...
  'options',jsonb_build_object(
    ...
    'hasEmptyLocation',EXISTS(SELECT 1 FROM marked WHERE location_label IS NULL AND pass_search AND pass_status AND pass_release AND pass_color AND pass_area AND pass_work_center AND pass_planning_group AND pass_ger AND pass_date AND pass_origin),
