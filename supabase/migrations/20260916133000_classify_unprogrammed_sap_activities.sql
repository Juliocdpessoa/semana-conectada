-- Separa apropriações SAP sem atividade programada em não programadas
-- (cód.plan.ordem 1) e imediatas (cód.plan.ordem 2).
...
WITH selected_week AS (
  ...
  SELECT s.id, s.order_number, s.operation, s.suboperation, s.description,
    ...
    s.planning_code,
    CASE
      WHEN nullif(regexp_replace(btrim(coalesce(s.planning_code, '')), '^0+', ''), '') = '2'
        THEN 'IMEDIATA'
      ELSE 'NÃO PROGRAMADA'
    END classification,
  ...
  WHERE s.import_id = (SELECT id FROM latest_import)
    ...
    AND nullif(regexp_replace(btrim(coalesce(s.planning_code, '')), '^0+', ''), '') IN ('1', '2')
