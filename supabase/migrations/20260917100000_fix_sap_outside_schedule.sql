-- Classifica apropriações fora da programação exclusivamente pela confirmação.
-- Ordem/operação iguais não tornam uma confirmação SAP automaticamente programada.
...
WITH selected_week AS (
  ...
  WHERE s.import_id = (SELECT id FROM latest_import)
    ...
    AND NOT EXISTS (
      SELECT 1
      FROM programmed_confirmations p
